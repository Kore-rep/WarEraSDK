 ...
 */
export const user = {
  getUserLite: async (baseUrl: string, id: string): Promise<GetUserLiteResponse> => {
    return request('user.getUserLite', { id }, baseUrl);
  },
  getUsersByCountry: async (baseUrl: string, countryId: string): Promise<UsersByCountryResponseDto> => {
