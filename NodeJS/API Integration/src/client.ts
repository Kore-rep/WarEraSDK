...
type ResourceMethods = Record<string, (baseUrl: string, ...args: never[]) => Promise<unknown>>;
...
export function createAPI(config: APIConfig = {}): APIClient {
  ...
  const createResourceWrapper = <T extends ResourceMethods>(resource: T): ResourceWrapper<T> => {
    const wrapper = {} as ResourceWrapper<T>;
    for (const [key, fn] of Object.entries(resource)) {
      if (typeof fn === 'function') {
        (wrapper as Record<string, (...args: unknown[]) => Promise<unknown>>)[key] = (...args: unknown[]) => 
          (fn as (baseUrl: string, ...args: unknown[]) => Promise<unknown>)(baseUrl, ...args);
      }
    }
    return wrapper;
  };
